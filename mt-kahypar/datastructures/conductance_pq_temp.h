...
public:
  ...
  void initialize(const PartitionedHypergraph& hg, bool synchronized = false, bool parallel = true) {
  ...
  std::vector<ConductanceInfo> topThree() const {
    /// [debug] std::cerr << "ConductancePriorityQueue::topThree()" << std::endl;
    std::vector<ConductanceInfo> top_three(3, ConductanceInfo());
  ...
  // ! (Works only for a binary heap)
  std::vector<ConductanceInfo> topThreeSync(bool synchronized = true) {
    /// [debug] std::cerr << "ConductancePriorityQueue::topThreeSync(" << V(synchronized) << ")" << std::endl;
    lock(synchronized);
    std::vector<ConductanceInfo> top_three = topThree();
    unlock(synchronized);
    return top_three;
  }
...
private:
  ...
  PartitionID _size;
  std::vector<bool> _complement_val_bits;
  std::vector<DeltaV> _delta_part_volumes;
  std::vector<DeltaV> _delta_cut_weights;
